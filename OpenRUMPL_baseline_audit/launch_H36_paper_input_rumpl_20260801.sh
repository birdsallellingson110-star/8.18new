#!/usr/bin/env bash
# H36: isolate Geometry-Biased Transformer's ray input inside the retained
# RUMPL backbone.  No set decoder, attention bias, token dropout, or temporal
# module is enabled in this input-only ablation.
set -euo pipefail

if [[ $# -ne 2 ]]; then
  echo "Usage: $0 {plucker|harmonic15|plucker_harmonic15} PHYSICAL_GPU" >&2
  exit 2
fi
...
tag="H36_${code}_${variant}_A1D_triAnchor_retainedRUMPL_curriculum_seed0_20260801"
...
# Retain the successful H0/H20 absolute-geometry path.
export RUMPL_TRI_ANCHOR=1
export RUMPL_TRI_ANCHOR_REG=1e-4
export RUMPL_TRI_ANCHOR_CONF_EPS=0.05

# The only manipulated variables in H36.
export RUMPL_INPUT_PLUCKER="${input_plucker}"
export RUMPL_INPUT_HARMONIC_L="${input_harmonic_l}"

# Disable architecture and loss confounders.
...
if [[ -z "${checkpoint}" ]]; then
  {
    echo "[H36] start tag=${tag}"
    echo "[H36] time=$(date --iso-8601=seconds) physical_gpu=${physical_gpu}"
    echo "[H36] input_only plucker=${input_plucker} harmonic_L=${input_harmonic_l}"
    echo "[H36] retained=A1D+triAnchor+RUMPL curriculum=fixed2_first8_then_3to1to1"
    echo "[H36] no_undistort: existing HRNet coordinates failed the undistortion audit"
  } | tee "${train_log}"
...
echo "[H36] end tag=${tag} time=$(date --iso-8601=seconds)" | tee -a "${train_log}"
