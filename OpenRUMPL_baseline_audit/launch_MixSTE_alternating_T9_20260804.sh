#!/usr/bin/env bash
# Follow-up: complete MixSTE STB<->TTB alternation while retaining RUMPL PFT.
set -euo pipefail
...
OUT=${ROOT}/MixSTE_alternating_T9_20260804
...
common=(
  ...
  --window-length 9 --frame-stride 5 --num-views 2 --depth 12 --heads 8
  --fusion-mode mixste-alternating --token-dropout 0.0
...
train_one A1_alternating_rumpl_mpjpe & p0=$!
train_one A2_alternating_mixste_original --loss-profile mixste-original & p1=$!
wait "${p0}"
wait "${p1}"
...
eval_one() {
  ...
  CUDA_VISIBLE_DEVICES=0 "${PY}" -u "${REPO}/run/eval_temporal_h36m_table2.py" \
    ...
    --fusion-mode mixste-alternating \
    --output-dir "${output}" --mmpose-type "${TYPE}" \
    --dataset-name annot_temporal_5_5 --num-views "${views}" \
    --window-length 9 --frame-stride 5 --depth 12 --heads 8 \
    --batch-size 16 --workers 4 --device cuda:0 \
    >"${output}/eval.log" 2>&1
}

for views in 2 3 4; do
  eval_one A1_alternating_rumpl_mpjpe "${views}" & q0=$!
  eval_one A2_alternating_mixste_original "${views}" & q1=$!
  wait "${q0}"
  wait "${q1}"
done
