#!/usr/bin/env bash
# Evaluate the completed residual-TTB screening with the same causal Table-2
# protocol and the already validated per-view frame caches.
set -euo pipefail
...
R1=${OUT}/R1_ttb_residual_rumpl_mpjpe/checkpoint_step_0003000.pth
R2=${OUT}/R2_ttb_residual_mixste_original/checkpoint_step_0003000.pth
declare -a p1 p2
for views in 2 3 4; do
  eval_one R1_ttb_residual_rumpl_mpjpe "${views}" "${R1}" & p1[${views}]=$!
  eval_one R2_ttb_residual_mixste_original "${views}" "${R2}" & p2[${views}]=$!
done
for views in 2 3 4; do
  wait "${p1[${views}]}"
  wait "${p2[${views}]}"
