...
OUTPUT=${ROOT}/H27_signed_context_gate
H21=${ROOT}/H21_pose_query_v2focus_reg005/final.pth
H26_GATE=${ROOT}/H26_context_anchor_gate/final.pth
...
if [[ ! -s "${OUTPUT}/final.pth" ]]; then
  "${PY}" -u "${AUDIT}/train_anchor_delta_gate.py" \
    ...
    --h21-checkpoint "${H21}" \
    --initial-gate-checkpoint "${H26_GATE}" \
    --context-features \
    ...
    --oracle-loss-weight 0.02 \
    --prior-loss-weight 0.002 \
    --seed 0 \
    --device cuda:0 \
    --output-dir "${OUTPUT}" \
...
echo "[H27] complete $(date --iso-8601=seconds)"
