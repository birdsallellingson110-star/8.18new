#!/usr/bin/env bash
# Re-evaluate the two trained raw-HRNet RUMPL controls with annotation boxes.
set -euo pipefail

if [[ $# -ne 2 ]]; then
  echo "Usage: $0 {F2|UK} PHYSICAL_GPU" >&2
  exit 2
fi
...
OUT=/mnt/data/cjyoutput/external_fair_comparison_20260813/raw_rumpl_h16
TYPE=mmpose_hrnet_coco_legswap

case "${variant}" in
  F2)
    pattern='H16_F2_R0_fixedK2_balancedCameraPairs_baseline_clean_realH36M_seed0_*'
    ;;
  UK)
    pattern='H16_UK_R0_uniformK2to4_balancedCameraSubset_baseline_clean_realH36M_seed0_*'
    ;;
  *)
...
# Match the original H16-R0 architecture.  Explicit values prevent unrelated
# experimental environment variables from leaking into this fair evaluation.
export GBT_LEARNABLE_BIAS=0 GBT_USE_CONF_BIAS=0 GBT_USE_GEOM_BIAS=0
export GBT_FUSION_GEOM=0 GBT_GLOBAL_JV_DEPTH=0 GBT_GLOBAL_JV_BIASED=0
export GBT_GLOBAL_JV_GATED=0 GBT_TOKEN_DROPOUT=0
