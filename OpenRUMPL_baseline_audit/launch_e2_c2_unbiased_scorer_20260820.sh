...
OUT=/mnt/data/cjyoutput/gbt_aligned_hrnet_20260820/e2_c2_unbiased_scorer
...
run_one() {
  ...
  if [[ -s "${dir}/result.json" ]]; then
    echo "[unbiased] seed${seed} already complete"
    return 0
  fi
  export CUDA_VISIBLE_DEVICES=${gpu}
  export PYTHONPATH=${AUDIT}
  "${PY}" -u "${AUDIT}/train_e2_c2_unbiased_scorer_20260820.py" \
    --train-shards "${TRAIN}" \
...
"${PY}" -u "${AUDIT}/evaluate_e2_c2_unbiased_scorer_20260820.py" \
  --cache "${VAL}" \
  --train-cache "${TRAIN}" \
  --checkpoint-root "${OUT}" \
...
echo "[unbiased] complete $(date --iso-8601=seconds)"
