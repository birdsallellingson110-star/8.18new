#!/usr/bin/env bash
# Stage-2 coordinate-only baseline: official Learnable Triangulation H36M
# ResNet-152 frontend, followed by the same RUMPL R0/H76 controls used for
# the GBT-aligned HRNet line.  The ResNet frontend is frozen; RUMPL receives
# only 2-D coordinates, confidences and the calibrated camera rays.
set -euo pipefail
...
OUT=/mnt/data/cjyoutput/gbt_aligned_resnet_20260817
...
export_frontend() {
  ...
  if [[ -s "${done}" && -s "${output}" && -s "${report}" ]]; then
    echo "[RES152] ${split} frontend already complete"
    return
  fi
  test -s "${input}"
  echo "[RES152] exporting ${split} on GPU ${gpu}"
...
if [[ ! -s "${FRONTEND}/train/export_complete.done" || ! -s "${FRONTEND}/validation/export_complete.done" ]]; then
  export_frontend train 0 & p_train=$!
  export_frontend validation 1 & p_val=$!
  wait "${p_train}" "${p_val}"
fi
...
run_one() {
  ...
  local tag="RES152_${variant}_LT_GBT_COORD_${TYPE}_seed0_20260817"
  local root="${OUT}/rumpl/${variant}"
  local log="${root}/${tag}.log"
  local done="${root}/${tag}.done"
  mkdir -p "${root}"
  if [[ -s "${done}" ]]; then
    echo "[RES152] ${variant} already complete"
    return
  fi
  (
...
run_one H76 1 1 1 1
wait
echo "[RES152] complete $(date --iso-8601=seconds)"
