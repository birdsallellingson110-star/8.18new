...
log="$run_root/watch_exact_baselines.log"

exec > >(tee -a "$log") 2>&1
echo "WATCH_START $(date --iso-8601=seconds)"

while tmux has-session -t rumpl_r0_exact_20260722 2>/dev/null || \
      tmux has-session -t rumpl_r1_exact_20260722 2>/dev/null; do
  echo "WAIT $(date --iso-8601=seconds)"
  sleep 60
done

status=0
for variant in R0_public_original_exact_seed0_20260722 R1_fix_pft_last_block_exact_seed0_20260722; do
