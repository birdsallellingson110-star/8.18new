...
def parse_args():
    ...
    parser.add_argument(
        '--n-views', help='number of cameras per all-camera test combination',
    ...
    parser.add_argument(
        '--test-occlusion-level', help='per-view structured 2D occlusion probability', type=float, default=None
    )
    parser.add_argument(
        '--test-occlusion-mode', help='occlusion mode: limb | random', type=str, default=None
    )
...
def reset_config(config, args):
    ...
    if args.test_occlusion_level is not None:
        config.DATASET.TEST_OCCLUSION_LEVEL = args.test_occlusion_level
    if args.test_occlusion_mode is not None:
        config.DATASET.TEST_OCCLUSION_MODE = args.test_occlusion_mode
...
def main():
    ...
    # Keep the checkpoint's training-time module shapes while allowing the
    # random-view fusion model to consume a different test combination size.
    if args.n_views is not None:
        config.DATASET.N_VIEWS_TRAIN_TEST_ALL = args.n_views
        logger.info('=> testing all-camera combinations with %d views', args.n_views)
