...
def main():
    ...
    init_checkpoint = os.environ.get("RUMPL_INIT_CHECKPOINT", "")
    if init_checkpoint:
    ...
    if os.environ.get("RUMPL_ADAPTER_ONLY", "0") == "1":
        trainable_fragments = (
            "joint_adapters",
            "joint_adapter_pos_embed",
            "joint_adapter_scale_raw",
        )
        for name, parameter in model.named_parameters():
            parameter.requires_grad = any(
                fragment in name for fragment in trainable_fragments
            )
        logger.info(
            "=> adapter-only trainable parameters: %d",
            sum(
                parameter.numel()
    ...
    elif os.environ.get("RUMPL_GLOBAL_ONLY", "0") == "1":
        trainable_fragments = ("global_joint",)
        for name, parameter in model.named_parameters():
            parameter.requires_grad = any(
                fragment in name for fragment in trainable_fragments
            )
        logger.info(
            "=> global-only trainable parameters: %d",
            sum(
                parameter.numel()
                for parameter in model.parameters()
                if parameter.requires_grad
    ...
    fix_scheduler_order = os.environ.get("RUMPL_FIX_SCHEDULER_ORDER", "0") == "1"
    logger.info('RUMPL_FIX_SCHEDULER_ORDER={}'.format(int(fix_scheduler_order)))
    ...
    for epoch in range(start_epoch, config.TRAIN.END_EPOCH):
        ...
        if os.environ.get("RUMPL_SAVE_EPOCH_STATES", "0") == "1":
            torch.save(
                model.module.state_dict(),
                os.path.join(
                    final_output_dir,
                    "epoch_{:02d}_state.pth.tar".format(epoch + 1),
                ),
            )
