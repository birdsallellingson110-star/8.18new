...
from utils.refine_2d_multiview import maybe_soft_refine_rays
...
from utils.calib import epiolar_error_between_cams
...
class MultiViewCMUpanoptic_RUMPL(JointsDataset_RUMPL):
    ...
    def __init__(self, cfg, image_set, is_train, transform=None, is_mmpose=False):
        ...
        if self.cmu_old_datasets:
            dataset_path = osp.join(self.root, 'data', self.dataset_type.replace('annot', 'annot_filtered'))
            if self.use_mmpose:
                dataset_path = osp.join(self.root, 'data', self.dataset_type.replace('annot', 'datasets_mmpose/annot_mmpose_filtered'))
        else:
            ...
            dataset_path = osp.join(self.root, 'data', dataset_folder_name, dataset_folder_name_2)
        ...
        if self.filter_cmu_wrong_cases and not self.test_multi_person:
        ...
        if not self.test_multi_person:
            super().do_mapping()

        if self.test_multi_person:
            self.grouping = self.get_group_multi_person(self.db)
        else:
        ...
        # structured 2D keypoint occlusion at test time (per-view limb dropout)
        self.test_occlusion_level = getattr(cfg.DATASET, 'TEST_OCCLUSION_LEVEL', 0.0)
        self.test_occlusion_mode = getattr(cfg.DATASET, 'TEST_OCCLUSION_MODE', 'limb')
        # COCO-17 limb groups
        self.occlusion_limb_groups = [
            [5, 7, 9],    # left arm
            [6, 8, 10],   # right arm
            [11, 13, 15], # left leg
            [12, 14, 16], # right leg
    ...
    def get_group_multi_person(self, db):
        grouping = {}
        nitems = len(db)
        if self.use_h36m_cameras_on_cmu:
            views = [3, 6, 12, 13, 23]
        ...
        for i in range(nitems):
            if not isinstance(db[i], list):
                db[i] = [db[i]]
            keystr = self.get_key_str(db[i][0])
            for j in range(len(db[i])):
                db[i][j]['annot_id'] = counter
                counter += 1
            if db[i][0]['camera_id'] not in views:
                continue
            camera_id = CAM_IX[db[i][0]['camera_id']]
        ...
        for i in range(len(filtered_grouping)):
            ids_cam = filtered_grouping[i]
            n_cams = len(ids_cam)
            n_person = max([len(db[ids_cam[j]]) for j in range(n_cams)])

            cost_matrix = np.zeros((n_person, n_person))
            for j in range(n_person):
                for k in range(n_person):
                    try:
                        cost_matrix[j, k] = epiolar_error_between_cams(
                                            db[ids_cam[0]][j]['camera'],
                                            db[ids_cam[1]][k]['camera'],
                                            db[ids_cam[0]][j]['joints_2d'],
                                            db[ids_cam[1]][k]['joints_2d'],
                                            db[ids_cam[0]][j]['joints_2d_conf'] if self.use_mmpose else np.ones((1, self.num_joints)),
                                            db[ids_cam[1]][k]['joints_2d_conf'] if self.use_mmpose else np.ones((1, self.num_joints)),
                                        )
                    except IndexError:
                        cost_matrix[j, k] = 1000000
            
            row_ind, col_ind = linear_sum_assignment(cost_matrix)

            for person_in_cam_1, person_in_cam_2 in zip(row_ind, col_ind):
                if cost_matrix[person_in_cam_1, person_in_cam_2] > 1000:
                    continue
                db_new.append(db[ids_cam[0]][person_in_cam_1])
                db_new.append(db[ids_cam[1]][person_in_cam_2])
                grouping_new.append([len(db_new) - 2, len(db_new) - 1])
                # if len(grouping_new) > 19:
                #     pass
    ...
    def __getitem__(self, idx):
        ...
        if self.normalize_room:
            ...
            if self.db[0]['source'] == 'cmu_panoptic':
        ...
        # Multi-view soft 2D/ray refine (env RUMPL_2D_REFINE=1). Runs AFTER
        # per-view occlusion / missing so occluded joints can be filled from
        # visible views. Default off keeps R5 unchanged.
        directions, joints_2d_confs_ = maybe_soft_refine_rays(
            directions, intersections, joints_2d_confs_
        )
