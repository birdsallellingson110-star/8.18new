...
class MultiViewRICH_RUMPL(JointsDataset_RUMPL):
    ...
    def __init__(self, cfg, image_set, is_train, transform=None, is_mmpose=False):
        ...
        self.rich_dataset_name = cfg.DATASET.TRAIN_RICH_DATASET_NAME if is_train else cfg.DATASET.TEST_RICH_DATASET_NAME
        ...
        dataset_folder_name_2 = self.rich_dataset_name + '_' + self.mmpose_type if self.use_mmpose else self.rich_dataset_name
        ...
        if self.val_on_train:
            anno_file = osp.join(dataset_path, 'rich_train.pkl')
        elif cfg.DATASET.CROP:
            anno_file = osp.join(dataset_path, 'rich_{}.pkl'.format(image_set))
        else:
        ...
        if self.run_on_all_cameras:
            ...
            for _, v in grouping.items():
                for view_comb in view_combinations:
                    if np.all(np.array([v[view] for view in view_comb]) != -1):
                        filtered_grouping.append([v[view] for view in view_comb])
        else:
            for _, v in grouping.items():
    ...
    def __getitem__(self, idx):
        ...
        for ix, item in enumerate(items):
            ...
            direction, intersection, joints_2d_conf, joints_3d, meta, joints_2d, K, Rt, depth_vals = self.get_rays(item, camera_id, shift_room_tri=shift_room_tri)
        ...
        # return middle_points, closest_points_all, joints_3d, rays, meta
