...
class MultiView_AMASS_RUMPL(JointsDataset_RUMPL):
    ...
    def __getitem__(self, idx):
        ...
        # Multi-view soft 2D/ray refine (env RUMPL_2D_REFINE=1). Same hook as CMU.
    ...
    def get_rays(self, joints_3d, camera_id, joints_3d_org=None, camera_setup_to_use=0, idx=None, shift_room_tri=None):
        ...
        # Stage 2a: structured limb-group occlusion augmentation (train only), mirroring the
        # test-time limb dropout in multiview_cmu_panoptic_rumpl. RUMPL's default Miss20 only
        # drops independent joints; whole-limb dropout is the failure mode responsible for the
        # occlusion gap (paper Sec 5.5). Env-gated so the R5 baseline stays unchanged by default.
        if self.is_train and os.environ.get("RUMPL_TRAIN_STRUCT_OCC", "0") == "1":
            struct_occ_level = float(os.environ.get("RUMPL_TRAIN_STRUCT_OCC_LEVEL", "0.4"))
            if np.random.uniform(0, 1) < struct_occ_level:
                limb_groups = [
                    [5, 7, 9],       # left arm
                    [6, 8, 10],      # right arm
                    [11, 13, 15],    # left leg
