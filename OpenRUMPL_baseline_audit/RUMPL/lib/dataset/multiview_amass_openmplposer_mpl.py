...
class MultiView_AMASS_OpenMPLPoser_MPL(JointsDataset_MPL):
    ...
    def __init__(self, cfg, image_set, is_train, transform=None, is_mmpose=False):
        ...
        self.openmplposer_cameras = self.load_all_cameras(openmplposer_calibs)
    ...
    def locate_person_in_the_room_val(self, db):
        if not self.output_in_meter:
            raise 'locate_person_in_the_room_val only works for output_in_meter=True'
        else:
    ...
    def __getitem__(self, idx):
        ...
        if not self.output_in_meter:
            raise 'output_in_meter must be True for OpenMPLPoser'
    ...
    def getitem(self, joints_3d, camera_id, joints_3d_org=None, camera_setup_to_use=0, idx=None):
        ...
        camera = self.openmplposer_cameras[camera_id][camera_setup_to_use].copy()
