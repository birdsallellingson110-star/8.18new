...
def symmetric_limb_loss(pose):
    ...
    loss = pose.new_zeros(())
    for left_start, left_end, right_start, right_end in symmetric_limbs:
        left_length = torch.linalg.vector_norm(
            pose[:, left_start] - pose[:, left_end], dim=-1
        )
        right_length = torch.linalg.vector_norm(
            pose[:, right_start] - pose[:, right_end], dim=-1
        )
        loss = loss + (left_length - right_length).square().mean()
    return loss
...
def train(config, data, model, criterion, optim, epoch, output_dir,
    ...
    for i, (middle_points, closest_points_all, target, rays, meta, joints_2ds) in enumerate(data):
        ...
        if config.NETWORK.FEED_CAMERA_CALIBRATION or config.NETWORK.FEED_ONLY_2D:
            ...
            if os.environ.get("GBT_LEARNED_RELIABILITY", "0") == "1":
                output, reliability_aux_loss = model(
                    rays, is_training=True, reliability_target=target
                )
            elif os.environ.get("RUMPL_RAY_DEPTH_AUX", "0") == "1":
                output, ray_depth_aux_loss = model(
                    rays, is_training=True, depth_target=target
                )
            elif os.environ.get("RUMPL_POSE_CODEBOOK", "0") == "1":
                output, pose_codebook_aux_loss = model(
                    rays, is_training=True, codebook_target=target
                )
            else:
        ...
        # else:
        # A2: occluded/low-visibility joint upweighted MPJPE (MPM-style recovery
        # supervision). Uses per-joint mean conf across views from ray input.
        # Env-gated; default off so R5 / S2a-alone stay unchanged.
        if os.environ.get("RUMPL_OCC_JOINT_LOSS", "0") == "1":
            boost = float(os.environ.get("RUMPL_OCC_JOINT_LOSS_BOOST", "2.0"))
            mode = os.environ.get("RUMPL_OCC_JOINT_LOSS_MODE", "soft")
            ray_conf = rays[..., -1].to(device=output.device, dtype=output.dtype)
            # (B, J, V) -> (B, J)
            mean_conf = ray_conf.clamp(0, 1).mean(dim=2)
            max_conf = ray_conf.clamp(0, 1).amax(dim=2)
            if mode == "hard":
                # only joints missing in ALL used views get boost
                joint_w = torch.where(max_conf < 0.01, boost, 1.0)
            else:
                # soft: less visible across views -> higher recovery weight
                joint_w = 1.0 + (boost - 1.0) * (1.0 - mean_conf)
            per_joint = torch.norm(output - target, dim=-1)
            loss = (joint_w * per_joint).mean()
            # keep names distinct from AverageMeter loss_x/y/z below
            axis_x = torch.mean(torch.abs(output[:, :, 0] - target[:, :, 0]))
            axis_y = torch.mean(torch.abs(output[:, :, 1] - target[:, :, 1]))
            axis_z = torch.mean(torch.abs(output[:, :, 2] - target[:, :, 2]))
            loss_axis = [axis_x, axis_y, axis_z]
        else:
            loss, loss_axis = criterion(output, target)
        if os.environ.get("GBT_LEARNED_RELIABILITY", "0") == "1":
            reliability_aux_weight = float(
                os.environ.get("GBT_RELIABILITY_AUX_WEIGHT", "1.0")
            )
            loss = loss + reliability_aux_weight * reliability_aux_loss.mean()
        if os.environ.get("RUMPL_RAY_DEPTH_AUX", "0") == "1":
            ray_depth_aux_weight = float(
                os.environ.get("RUMPL_RAY_DEPTH_AUX_WEIGHT", "0.1")
            )
            loss = loss + ray_depth_aux_weight * ray_depth_aux_loss.mean()
        if os.environ.get("RUMPL_POSE_CODEBOOK", "0") == "1":
            pose_codebook_aux_weight = float(
                os.environ.get("RUMPL_POSE_CODEBOOK_COMMIT_W", "0.1")
            )
            loss = loss + pose_codebook_aux_weight * pose_codebook_aux_loss.mean()
        symmetry_weight = float(
            os.environ.get("RUMPL_SYMMETRY_LOSS_WEIGHT", "0.0")
        )
        if symmetry_weight > 0:
            loss = loss + symmetry_weight * symmetric_limb_loss(output)
        # ===== full-view -> sparse-view self-distillation (env DISTILL_W>0) =====
        _dw = float(os.environ.get('DISTILL_W', os.environ.get('DISTILL_LAMBDA', 0.0)))
        _fw = float(os.environ.get('FEAT_DISTILL_W', 0.0))
        _gw = float(os.environ.get('STUDENT_GT_W', 1.0)) if (
            _dw > 0 or _fw > 0 or os.environ.get('STUDENT_GT_W')
        ) else 0.0
        if (_dw > 0 or _fw > 0 or _gw > 0) and rays.shape[2] >= 3:
            ...
            _bb = getattr(model, 'module', model)
            _bb = getattr(_bb, 'features', _bb)
            if _fw > 0 and not hasattr(_bb.head, '_feat_hooked'):
                def _grab(mod, inp, out):
                    mod._feat_in = inp[0]
                _bb.head.register_forward_hook(_grab)
                _bb.head._feat_hooked = True
            with torch.no_grad():
                _teach = model(rays[:, :, :_vt, :], is_training=False)
            ...
            if os.environ.get('HARD_VIEW_MINING', '0') == '1' and _fw == 0:
                ...
                for _pi in _order:
                    _pair = _pairs[_pi]
                    _cand = model(rays[:, :, list(_pair), :], is_training=False)
                    _score = (((_cand - _teach.detach()) ** 2).sum(-1) + 1e-8).sqrt().mean()
                    if _best_score is None or _score.detach() > _best_score:
                        _best_score, _best_pair = _score.detach(), _pair
                ...
                _stud = model(rays[:, :, _idx, :], is_training=False)
            else:
                _stud = model(rays[:, :, :_ks, :], is_training=False)
            _feat_s = _bb.head._feat_in if _fw > 0 else None
            _dist_pj = (((_stud - _teach.detach()) ** 2).sum(-1) + 1e-8).sqrt()
            _legw = float(os.environ.get('LEG_DISTILL_W', 1.0))
            if _legw != 1.0:
                _jw = torch.ones(17, device=_dist_pj.device)
                _jw[[11, 12, 13, 14, 15, 16]] = _legw
                _d = (_dist_pj * _jw).mean()
            else:
                _d = _dist_pj.mean()
            _g = (((_stud - target) ** 2).sum(-1) + 1e-8).sqrt().mean()
            _dfeat = ((_feat_s - _feat_t) ** 2).mean() if _fw > 0 else 0.0
            if i % 1000 == 0:
                _fv = _dfeat.item() if _fw > 0 else 0.0
                print(
                    f"[DISTILL] iter {i}: main={loss.item():.4f} d(s-t)={_d.item():.4f} "
                    f"g={_g.item():.4f} feat={_fv:.4f} dw={_dw} fw={_fw} gw={_gw} legw={_legw} "
                    f"k={_ks} vt={_vt} hard_pair={_hard_pair} hard_views={_hard_views}",
                    flush=True,
                )
            loss = loss + _gw * _g + _dw * _d + _fw * _dfeat
        # target = t
    ...
    with torch.no_grad():
        ...
        for i, (middle_points, closest_points_all, target, rays, meta, joints_2ds) in enumerate(loader):
            ...
            if config.NETWORK.FEED_CAMERA_CALIBRATION or config.NETWORK.FEED_ONLY_2D:
                ...
                model_kwargs = {}
                if os.environ.get("GBT_COARSE_RELIABILITY", "0") == "1":
                    coarse_output = model(
                        rays, is_training=False, disable_reliability=True
                    )
                    model_kwargs["oracle_target"] = coarse_output.detach()
                elif os.environ.get("GBT_ORACLE_RELIABILITY", "0") == "1":
                    model_kwargs["oracle_target"] = target
                output = model(rays, is_training=False, **model_kwargs)
            elif config.NETWORK.POSE_3D_FUSER_USE_MIDDLE_POINTS:
                output = model(middle_points)
            else:
                output = model(closest_points_all)
