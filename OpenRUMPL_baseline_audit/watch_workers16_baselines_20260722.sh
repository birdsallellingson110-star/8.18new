...
log="$run_root/watch_workers16_baselines.log"

exec > >(tee -a "$log") 2>&1
echo "WATCH_START $(date --iso-8601=seconds)"

while tmux has-session -t rumpl_r4_workers16_original_20260722 2>/dev/null || \
      tmux has-session -t rumpl_r5_workers16_scheduler_20260722 2>/dev/null; do
  echo "WAIT $(date --iso-8601=seconds)"
  sleep 60
done

status=0
for variant in R4_workers16_public_original_exact_seed0_20260722 R5_workers16_fix_scheduler_exact_seed0_20260722; do
