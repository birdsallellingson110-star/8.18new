#!/usr/bin/env bash
# H37: move joint/view interaction before RUMPL's per-joint fusion collapse.
# The plain variant isolates the global joint-view block; confgeom additionally
# applies the GBT confidence and ray-distance biases at that global location.
set -euo pipefail

if [[ $# -ne 2 ]]; then
  echo "Usage: $0 {plain|confgeom} PHYSICAL_GPU" >&2
  exit 2
fi
...
tag="H37_${code}_globalJV1_${variant}_A1D_triAnchor_retainedRUMPL_seed0_20260801"
...
# Keep the strongest controlled baseline unchanged.
export RUMPL_TRI_ANCHOR=1
export RUMPL_TRI_ANCHOR_REG=1e-4
export RUMPL_TRI_ANCHOR_CONF_EPS=0.05

# H37 manipulated variables.  ReZero gating makes J0 exactly baseline-like at
# initialization and tests whether cross-joint/cross-view interaction is useful.
export GBT_GLOBAL_JV_DEPTH=1
export GBT_GLOBAL_JV_GATED=1
export GBT_GLOBAL_JV_BIASED="${biased}"
export GBT_CONF_INIT=0.1
export GBT_GEOM_INIT=0.1

# Disable all other experimental branches and auxiliary losses.
...
if [[ -z "${checkpoint}" ]]; then
  {
    echo "[H37] start tag=${tag}"
    echo "[H37] time=$(date --iso-8601=seconds) physical_gpu=${physical_gpu}"
    echo "[H37] global_jv_depth=1 gated=1 biased=${biased}"
    echo "[H37] retained=A1D+triAnchor+RUMPL curriculum=fixed2_first8_then_3to1to1"
  } | tee "${train_log}"

  "${PY}" -u run/train_rumpl.py \
    --cfg "${CFG}" --gpus 0 --workers "${WORKERS}" \
...
echo "[H37] end tag=${tag} time=$(date --iso-8601=seconds)" | tee -a "${train_log}"
