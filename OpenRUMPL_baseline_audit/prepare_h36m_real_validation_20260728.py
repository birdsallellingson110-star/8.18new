#!/usr/bin/env python3
"""Convert the downloaded H36M h5/JPG bundle to RUMPL validation PKL."""
...
def parse_args():
    ...
    # The shared JPG/H5 bundle contains every fifth original frame. Taking
    # every 13th available multiview group approximates the paper's val stride
    # of 64 original frames (5 * 13 = 65).
    parser.add_argument("--available-group-stride", type=int, default=13)
    parser.add_argument("--dataset-name", default="annot_filtered_5_64")
    return parser.parse_args()


def load_take_mapping(toolbox_root):
    old_cwd = os.getcwd()
    try:
...
def main():
    ...
    take_mapping = load_take_mapping(toolbox_root)
    ...
    with (annot_root / "valid_images.txt").open() as handle:
    ...
    for row_index, image_name in enumerate(image_names):
        ...
        parsed.append(item)
        if is_damaged(subject, action, subaction):
            continue
    ...
    selected_groups = complete_groups[:: args.available_group_stride]
    selected_rows = {
        row_index
        for _, rows in selected_groups
        for row_index in rows.values()
    }

    records = []
    with h5py.File(annot_root / "valid.h5", "r") as h5_file:
        for item in parsed:
            ...
            crop_center = np.asarray(
                h5_file["center"][row_index], dtype=np.float64
            )
            crop_scale_scalar = float(h5_file["scale"][row_index])
            crop_scale = np.array(
                [crop_scale_scalar, crop_scale_scalar], dtype=np.float64
            )
            ...
            # RUMPL's H36M per-action evaluator reads the numeric action from
            # the fourth underscore-delimited filename field.  Keep the
            # downloaded image untouched and expose an evaluation-compatible
            # symlink name.
            eval_image = (
    ...
    output_dir = (
        h36m_root / "data" / "datasets" / args.dataset_name
    )
    output_dir.mkdir(parents=True, exist_ok=True)
    output_file = output_dir / "h36m_validation.pkl"
    with output_file.open("wb") as handle:
        pickle.dump(records, handle, protocol=pickle.HIGHEST_PROTOCOL)

    summary = {
        "source_rows": len(image_names),
        "complete_groups_after_damage_filter": len(complete_groups),
        "available_group_stride": args.available_group_stride,
        "selected_groups": len(selected_groups),
        "selected_records": len(records),
        "selected_subjects": sorted({record["subject"] for record in records}),
