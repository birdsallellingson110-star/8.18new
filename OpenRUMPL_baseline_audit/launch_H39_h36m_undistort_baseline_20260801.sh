#!/usr/bin/env bash
# H39: controlled H0 rerun with only H36M 2D point undistortion enabled.
set -euo pipefail
...
# The only manipulated variable relative to H0.
export RUMPL_UNDISTORT_H36M_POINTS=1

# Retain the H0 RUMPL body-prior/residual-decoder path.
...
if [[ -z "${checkpoint}" ]]; then
  {
    echo "[H39] start tag=${tag}"
    echo "[H39] time=$(date --iso-8601=seconds) physical_gpu=${physical_gpu}"
    echo "[H39] only_change=H36M_cv2_undistortPoints_before_pinhole_rays"
    echo "[H39] retained=A1D+triAnchor+original_RUMPL curriculum=fixed2_first8_then_3to1to1"
  } | tee "${train_log}"
...
for n_views in 2 3 4; do
  ...
  "${PY}" -u run/eval_rumpl_checkpoint.py \
    ...
    --workers "${WORKERS}" --gpu 0 --use-mmpose-val true \
    --flip-lower-body-kp-test true --test-views "${test_views[@]}" \
    --test-on-all-cameras true --test-mmpose-type "${TYPE}" \
...
echo "[H39] end tag=${tag} time=$(date --iso-8601=seconds)" | tee -a "${train_log}"
