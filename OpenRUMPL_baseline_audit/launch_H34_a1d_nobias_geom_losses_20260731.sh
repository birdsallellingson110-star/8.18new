#!/usr/bin/env bash
# H34: A1D + tri-anchor + light bone/reproj/ray, NO GBT bias (H22 curriculum).
set -euo pipefail
...
if [[ -z "${checkpoint}" ]]; then
  {
    echo "[H34] start ${tag} $(date --iso-8601=seconds)"
    echo "[H34] A1D+triAnchor NO bias | bone/reproj/ray=0.01"
  } | tee "${train_log}"
