#!/usr/bin/env bash
# H13: MTF source-normalized pairwise relation as a zero-initialized
# per-view residual before the original RUMPL fusion-token VFT.
set -euo pipefail
...
OUT=/mnt/data/cjyoutput/gbt_aligned_hrnet_20260817/h13_mtf_source_norm_residual_ab
...
for split in train validation; do
  ...
  if [[ -e "${target}" || -L "${target}" ]]; then
    ...
    [[ "${existing}" == "${expected}" ]] || {
      echo "[H13] refusing to replace ${target} -> ${existing}; expected ${expected}" >&2
      exit 1
    }
  else
    ln -s "${source}" "${target}"
...
exec > >(tee -a "${LOG}") 2>&1
echo "[H13] start $(date --iso-8601=seconds)"
echo "[H13] train=${TRAIN_MERGED}"
echo "[H13] val=${VAL_MERGED}"
sha256sum "${CFG}" "${TRAIN_MERGED}" "${VAL_MERGED}"

run_one() {
  local variant=$1 gpu=$2 seed=$3 use_conf=$4
  local tag="H13_${variant}_MTFSourceNormResidual_HRNet_seed${seed}_20260817"
  local root="${OUT}/${variant}"
  local log="${root}/${tag}.log"
  local done="${root}/${tag}.done"
  mkdir -p "${root}"
  if [[ -s "${done}" ]]; then
    echo "[H13] ${variant} already done"
    return 0
  fi
  (
    ...
    export RUMPL_MTF_SOURCE_NORM_RESIDUAL=1
    export RUMPL_MTF_SOURCE_NORM_CONFIDENCE="${use_conf}"
    {
      ...
      echo "MTF source-normalized residual before original VFT; confidence=${use_conf}; camera_id=0"
      sha256sum "${CFG}" "${TRAIN_MERGED}" "${VAL_MERGED}"
    } >"${log}"
    cd "${REPO}"
...
run_one RESIDUAL_PLAIN 0 0 0
run_one RESIDUAL_CONF 1 1 1
wait
echo "[H13] complete $(date --iso-8601=seconds)"
