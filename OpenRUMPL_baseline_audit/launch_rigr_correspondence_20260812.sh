...
mkdir -p "${ROOT}/correspondence_seed0" "${ROOT}/correspondence_bias_seed1"
...
run_one() {
  ...
  local extra=()
  if [[ "${variant}" == "correspondence_bias_seed1" ]]; then
...
run_one 0 0 correspondence_seed0 &
pid0=$!
run_one 1 1 correspondence_bias_seed1 &
pid1=$!
wait "${pid0}"
wait "${pid1}"
date '+%F %T RIGR cross-view correspondence pair completed'
