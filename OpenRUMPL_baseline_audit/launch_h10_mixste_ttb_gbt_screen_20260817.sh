#!/usr/bin/env bash
# H10: MixSTE per-joint TTB after RUMPL VFT, retained as a scaled residual.
# This is a separate, frozen-backbone control against H9's pose-space residual.
set -euo pipefail
...
OUT=/mnt/data/cjyoutput/gbt_aligned_hrnet_20260817/h10_mixste_ttb_residual
STEPS=${H10_STEPS:-12000}
...
{
  echo "arm=H10_MIXSTE_TTB_RESIDUAL gpu=1 start=$(date --iso-8601=seconds)"
  echo "base=${BASE} train_type=${TYPE} train_dataset=${TRAIN_NAME}"
  echo "T=9 stride=5 random-K2; MixSTE TTB after RUMPL VFT; frozen RUMPL; steps=${STEPS}"
  sha256sum "${CFG}" "${BASE}"
} >"${OUT}/logs/H10_MIXSTE_TTB_RESIDUAL.log"
...
"${PY}" -u run/train_temporal_gbt_rumpl.py \
  ...
  --fusion-mode mixste-ttb-residual --depth 4 --heads 8 --token-dropout 0 \
  ...
  --loss-profile rumpl --loss-type mpjpe --loss-frame all \
  >>"${OUT}/logs/H10_MIXSTE_TTB_RESIDUAL.log" 2>&1
