#!/usr/bin/env bash
# R5b: H76 frozen + zero-output token-conditioned MLP driven by ray consistency.
set -euo pipefail
...
export CODE_OVERRIDE="R5b_${variant}_seed${seed}"
export BASE_OVERRIDE="${ROOT}/R5b_joint_geometry_conditional_residual_20260809"
export TAG_OVERRIDE="R5b_H76_jointGeometryConditionalResidual_w322_${variant}_seed${seed}_20260809"
export CONTROL_NOTE_OVERRIDE="H76 frozen; zero-output token-conditioned MLP driven by normalized ray consistency; 5 epochs; lr=${lr}"
...
export RUMPL_JOINT_GEOMETRY_CONDITIONAL_RESIDUAL=1
export RUMPL_TRAIN_SCOPE=joint_geometry_conditional_residual
