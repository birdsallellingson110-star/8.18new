...
OUTPUT=/mnt/data/cjyoutput/open_source_fusion_audit_20260731/Res152_square_val_20260814
...
CHECKPOINT=/mnt/data/cjyoutput/open_source_fusion_audit_20260731/pretrained/td-hm_res152_8xb32-210e_coco-384x288-7fbb906f_20220927.pth
NUM_SHARDS=4
...
run_shard() {
  ...
  echo "start ResNet-152 square validation shard ${shard_id}/${NUM_SHARDS} on GPU ${gpu}"
...
if (( status != 0 )); then
  echo 'at least one ResNet-152 validation shard failed' >&2
  exit "${status}"
fi
echo 'ResNet-152 square validation export complete'
