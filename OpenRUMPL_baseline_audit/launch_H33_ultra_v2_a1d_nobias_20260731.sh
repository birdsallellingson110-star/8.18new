#!/usr/bin/env bash
# H33: A1D 2D + tri-anchor + ultra-V2 curriculum, NO GBT bias
# (H32 showed conf/geom bias regresses vs plain H0 on A1D data).
set -euo pipefail
...
# Explicitly disable bias — H32 regression lesson.
...
if [[ -z "${checkpoint}" ]]; then
  {
    ...
    echo "[H33] TARGETS V2<40 V4<30 | A1D+triAnchor NO bias | fixed2=${fixed_epochs} then ${view_count_weights}"
    echo "[H33] note: H32 confGeom regressed vs H0; this run isolates ultra-V2 curriculum"
  } | tee "${train_log}"
