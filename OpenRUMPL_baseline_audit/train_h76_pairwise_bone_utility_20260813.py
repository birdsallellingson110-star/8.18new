#!/usr/bin/env python3
"""Pairwise E2 utility with a staged weak bone-length consistency loss.

The model and candidate pool are identical to pairwise E2.  During the direct
counterfactual stage the loss is unchanged.  During the GHT fine-tuning stage,
the soft fused pose additionally matches the training pose's 16 skeleton bone
lengths.  This tests the kinematic-consistency failure mode introduced by
per-joint candidate weights, following the bone prior used by FLEX/graph-based
multi-view pose methods.  No bone length or GT is used at inference.
"""
...
# 17-joint H36M topology used by RUMPL's COCO/H36M joint order.
BONE_EDGES = (
    (0, 1), (1, 2), (2, 3),
    (0, 4), (4, 5), (5, 6),
    (0, 7), (7, 8), (8, 9), (9, 10),
...
def bone_lengths(pose: torch.Tensor) -> torch.Tensor:
    first = pose[..., [a for a, _ in BONE_EDGES], :]
    second = pose[..., [b for _, b in BONE_EDGES], :]
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--bone-weight", type=float, required=True)
...
def task_loss(model, predictions, targets, rays, phase, temperature,
              target_temperature_m, oracle_weight, bone_weight):
    direct, oracle, ght, bone_terms = [], [], [], []
    target_bone = bone_lengths(targets)
    for task in TASK_COMBINATIONS:
        ...
        if phase == "ght":
            ...
            # 0.01 m normalization keeps this term comparable to the GHT
            # expected-risk scale.  The coefficient is predeclared per run.
            fused_bone = bone_lengths(fused)
            bone_terms.append(
                F.smooth_l1_loss(fused_bone, target_bone, beta=0.01) / 0.01
            )
    loss = torch.stack(direct).mean() + oracle_weight * torch.stack(oracle).mean()
    if ght:
        loss = loss + torch.stack(ght).mean()
        loss = loss + bone_weight * torch.stack(bone_terms).mean()
    return loss


def main() -> None:
    args = parse_args()
    if args.bone_weight < 0 or args.target_temperature_mm <= 0:
        raise ValueError("bone weight must be non-negative and target temperature positive")
    ...
    for epoch, (phase, lr) in enumerate(phases):
        ...
        if metric < best_metric:
            ...
            torch.save({
                ...
                "temperature": args.temperature, "bone_weight": args.bone_weight,
                "method": "pairwise_e2_staged_bone_length",
            }, checkpoint)
    ...
    result = {
        "method": "pairwise E2 + staged weak bone-length consistency",
        "paper_basis": "FLEX-style bone prior and graph-based pose structure; no inference GT",
