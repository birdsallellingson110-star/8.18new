#!/usr/bin/env bash
# Follow-up to C1: same single B2 checkpoint and mixed cardinality sampler,
# with two explicitly separated optimization schedules.  This is a
# cardinality-recovery control, not a multi-model ensemble.
set -euo pipefail
...
OUT=/mnt/data/cjyoutput/gbt_aligned_hrnet_20260815/cardinality_recovery_v2
mkdir -p "${OUT}"
test -s "${CFG}" && test -s "${TRAIN}" && test -s "${VAL}" && test -s "${B2}"

run_one() {
  local variant="$1" gpu="$2" end_epoch="$3" lr="$4" lr_steps="$5"
  local tag="CARD2_${variant}_B2_MIXED_T1_${end_epoch}E_seed0_20260815"
  ...
  (
    ...
    export RUMPL_FINETUNE_LR="${lr}" RUMPL_INIT_CHECKPOINT="${B2}"
    ...
    export MONO_W=0 MONO_GT_W=0 RUMPL_TRAIN_SCOPE=all
    {
      ...
      echo "init=${B2} weights=3,1,1 end_epoch=${end_epoch} lr=${lr} lr_steps=${lr_steps}"
      echo "input_type=${TYPE} train=${TRAIN} val=${VAL} flip=false"
...
run_one LONG_LR1E5 0 60 1e-5 30,45
run_one HIGH_LR5E5 1 20 5e-5 10,15
wait
echo "[CARD2] complete $(date --iso-8601=seconds)"
