#!/usr/bin/env bash
# Restore HRNet V3/V4 exposure without abandoning canonical coordinates/token10.
# Branch selection is performed on S8 only; S9/S11 is evaluated once afterward.
set -euo pipefail
...
ROOT=/mnt/data/cjyoutput/camera_generalization_20260824/hrnet_highview_restore_20260826
...
run_branch() (
  set -euo pipefail
  local short_name=$1 weights=$2
  local repair_name=highview_${short_name}_20260826
  REPAIR_NAME="${repair_name}" \
  REPAIR_INIT="${CONTROL}" \
  REPAIR_LR=1e-6 REPAIR_EPOCHS=6 REPAIR_LR_STEPS=4 \
  REPAIR_PELVIS_PRIOR=1 REPAIR_BODY_REG=1e-2 REPAIR_VISIBLE_GPU=1 \
  REPAIR_VIEW_COUNT_WEIGHTS="${weights}" \
  REPAIR_GBT_TOKEN_DROPOUT=0.10 REPAIR_GBT_TOKEN_DROPOUT_EPOCHS=2 \
...
run_branch restore_433 4,3,3 & p0=$!
run_branch restore_244 2,4,4 & p1=$!
failed=0
wait "${p0}" || failed=1
wait "${p1}" || failed=1
(( failed == 0 ))
...
set_eval_environment
for name in token10_control restore_433 restore_244; do
  case "${name}" in
    token10_control) checkpoint=${CONTROL} ;;
...
"${PY}" "${AUDIT}/select_hrnet_highview_restore_s8_20260826.py" \
  >"${ROOT}/selection_s8.log" 2>&1
selected=$(cat "${ROOT}/selected_checkpoint.txt")

# Exactly one S9/S11 evaluation, after S8-only selection.
...
sha256sum "${CONTROL}" "${ROOT}/restore_433/final_checkpoint.txt" \
  "${ROOT}/restore_244/final_checkpoint.txt" "${ROOT}/selection_s8.json" \
  >"${ROOT}/audit.sha256"
date --iso-8601=seconds >"${ROOT}/COMPLETED"
echo "[HRNet high-view restore] complete ${ROOT}"
