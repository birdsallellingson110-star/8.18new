...
if [[ "${SEED}" -eq 0 ]]; then
  OUTPUT=${ROOT}/H24_learned_anchor_gate
else
  OUTPUT=${ROOT}/H24_learned_anchor_gate_seed${SEED}
fi
...
for views in 2 3 4; do
  while [[ ! -s "${CACHE}/V${views}/preds_gt_multiview_h36m_rumpl_mmpose__dict.pkl" \
        || ! -s "${CACHE}/V${views}/groups.json" ]]; do
    sleep 30
  done
done
...
echo "[H24] complete $(date --iso-8601=seconds)"
