#!/usr/bin/env bash
# H38: independent RUMPL fusion/representation ablations on the H0 baseline.
set -euo pipefail

if [[ $# -ne 2 ]]; then
  echo "Usage: $0 {relative|center|relative_center} PHYSICAL_GPU" >&2
  exit 2
fi
...
tag="H38_${code}_${variant}_A1D_triAnchor_retainedRUMPL_seed0_20260801"
...
if [[ -z "${checkpoint}" ]]; then
  {
    echo "[H38] start tag=${tag}"
    echo "[H38] time=$(date --iso-8601=seconds) physical_gpu=${physical_gpu}"
    echo "[H38] relative_view=${relative} root_anchor_centered=${centered}"
    echo "[H38] retained=A1D+triAnchor+RUMPL curriculum=fixed2_first8_then_3to1to1"
  } | tee "${train_log}"
...
echo "[H38] end tag=${tag} time=$(date --iso-8601=seconds)" | tee -a "${train_log}"
