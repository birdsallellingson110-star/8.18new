...
if __name__ == '__main__':
    ...
    model = Model(num_frame=opt.frames, num_joints=17, in_chans=2, embed_dim_ratio=opt.embed_dim_ratio, depth=opt.depth,
                      num_heads=8, mlp_ratio=2., qkv_bias=True, qk_scale=None, drop_path_rate=0.)

    if torch.cuda.device_count() > 1:
        print("Let's use", torch.cuda.device_count(), "GPUs!")
        model = torch.nn.DataParallel(model, device_ids=opt.gpu).to(device)
