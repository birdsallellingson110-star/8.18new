...
def get_rays_new(image_size, H, W, K, R, T, ret_rays_o=False):
    ...
    batch = 1  # 单张图片
    views = 1  # 单个视角
    K = torch.tensor(K).reshape(1, 3, 3).float()  # 转换为PyTorch张量
    R = torch.tensor(R).reshape(1, 3, 3).float()  # 转换为PyTorch张量
    T = torch.tensor(T).reshape(1, 3, 1).float()  # 转换为PyTorch张量
    K[:, :2] *= ratio
    rays_o = -torch.bmm(R.transpose(2, 1), T)
    j, i = torch.meshgrid(torch.linspace(0, H - 1, H),
                          torch.linspace(0, W - 1, W))
    xy1 = torch.stack([i, j, torch.ones_like(i)], dim=-1).unsqueeze(0)
    pixel_camera = torch.bmm(xy1.flatten(1, 2), torch.inverse(K).transpose(2, 1))
