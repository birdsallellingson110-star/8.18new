...
# Unified sparse-view distillation:
# sample K in [2, vt-1], choose the hardest 2-view pair against the full-view
# teacher, then add random extra views for K>2.
export DISTILL_W=1
...
exec /home/lixiaob/cjy/rumpl_venv310/bin/python run/train_rumpl.py \
  ...
  --gpus 0 \
  --workers 8 \
  --exp-name distill_multik_hard_legw09_nogbt_20260714
