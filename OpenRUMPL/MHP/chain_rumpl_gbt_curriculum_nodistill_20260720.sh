...
TRAIN=/home/lixiaob/cjy/OpenRUMPL/MHP/run_rumpl_gbt_curriculum_nodistill_20260720.sh
...
REPORT=/mnt/data/cjyoutput/rumpl_gbt_curriculum5_nodistill_eval_summary_20260720.txt
LOG=/mnt/data/cjyoutput/rumpl_gbt_curriculum5_nodistill_chain_20260720.log
...
exec > >(tee -a "$LOG") 2>&1
echo "[train] $(date '+%F %T') five-epoch V2 to random-K curriculum"
pids=()
for spec in "${RUNS[@]}"; do
...
find_run() {
  ...
  find "$ROOT" -maxdepth 1 -type d \
    -name "rumpl_gbt_curriculum5_nodistill_${mode}_20260720_*" -print | sort | tail -n1
}
...
for spec in "${RUNS[@]}"; do
  ...
  name="rumpl_gbt_curriculum5_nodistill_${mode}_final20"
...
for spec in "${RUNS[@]}"; do
  IFS=: read -r mode gpu <<< "$spec"
  name="rumpl_gbt_curriculum5_nodistill_${mode}_final20"
  control="/mnt/data/cjyoutput/cmu_v2_eval_${name}_20260720"
  for k in 2 3 4 5; do
    candidate="/mnt/data/cjyoutput/cmu_v${k}_eval_${name}_20260720"
    curriculum_control="/mnt/data/cjyoutput/cmu_v${k}_eval_rumpl_gbt_curriculum5_nodistill_none_final20_20260720"
    echo "=== ${name} V${k} ===" >> "$REPORT"
    if [[ $k == 2 ]]; then
      "$PY" "$SUMMARY" "$candidate" --rumpl-values "$V2_RUMPL" \
        --baseline "$curriculum_control" --name "$name" >> "$REPORT"
    elif [[ $k == 3 ]]; then
