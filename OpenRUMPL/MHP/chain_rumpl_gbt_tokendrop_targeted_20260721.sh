...
REPORT=/mnt/data/cjyoutput/rumpl_gbt_tokendrop_targeted_20260721.txt
LOG=/mnt/data/cjyoutput/rumpl_gbt_tokendrop_targeted_20260721.log
V2_RUMPL='3_6=40.37,3_12=46.95,3_13=39.79,3_23=32.30,6_12=67.28,6_13=53.52,6_23=39.39,12_13=59.41,12_23=46.04,13_23=44.08'

exec > >(tee -a "$LOG") 2>&1
echo "[start] $(date '+%F %T') targeted token-removal"
GBT_TOKEN_DROPOUT_EPOCHS=5 TOKEN_DROP_TAG=_stage5 "$TRAIN" 0.1 0 & p1=$!
GBT_TOKEN_DROPOUT_EPOCHS=0 TOKEN_DROP_TAG=_rate05 "$TRAIN" 0.05 1 & p2=$!
wait "$p1"
wait "$p2"

evaluate_variant() {
  local pattern=$1 name=$2 gpu=$3 run checkpoint k
  run=$(find "$ROOT" -maxdepth 1 -type d -name "$pattern" -print | sort | tail -n1)
...
evaluate_variant 'rumpl_gbt_curriculum5_tokendrop0p1_stage5_seed0_20260721_*' \
  rumpl_gbt_tokendrop0p1_stage5_seed0_final20 0 & e1=$!
evaluate_variant 'rumpl_gbt_curriculum5_tokendrop0p05_rate05_seed0_20260721_*' \
  rumpl_gbt_tokendrop0p05_all_seed0_final20 1 & e2=$!
wait "$e1"
wait "$e2"

: > "$REPORT"
for variant in stage5 rate05; do
  if [[ $variant == stage5 ]]; then
    name=rumpl_gbt_tokendrop0p1_stage5_seed0_final20
  else
    name=rumpl_gbt_tokendrop0p05_all_seed0_final20
  fi
  for k in 2 3 4 5; do
    ...
    echo "=== variant=${variant} V${k} ===" >> "$REPORT"
    if [[ $k == 2 ]]; then
