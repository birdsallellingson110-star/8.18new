...
def sample_clips_from_npz(npz_path, L_clip, stride, keep_rate, central_pct=0.8, rng=None, motion_sampling=False):
    ...
    if motion_sampling:
        # 优先采运动幅度大的片段 (pose帧间L1变化大 = 肢体动作丰富 = 时序信号强)。
        # ST-VFT 学帧间变化, 静止 clip 无用 → 按 pose 帧间变化排序选 top keep_n。
        _poses = cdata['poses']
        def _motion(s):
            return float(np.abs(np.diff(_poses[s:s + L_clip], axis=0)).mean())
        sampled = sorted(sorted(valid_starts, key=_motion, reverse=True)[:keep_n])
    else:
...
def main():
    ...
    ap.add_argument('--motion-sampling', action='store_true',
                    help='优先采运动幅度大的clip(ST-VFT时序需要); 默认随机采')
    args = ap.parse_args()
