...
TRAIN=/home/lixiaob/cjy/OpenRUMPL/MHP/run_distill_hardv_legw09_gbt_learnable_20260719.sh
...
READY=/mnt/data/cjyoutput/kaware_conf_eval_summary_20260719.txt
REPORT=/mnt/data/cjyoutput/gbt_learnable_unified_eval_summary_20260720.txt
CHAIN_LOG=/mnt/data/cjyoutput/gbt_learnable_unified_chain_20260719.log
...
echo "[wait] $(date '+%F %T') waiting for preliminary runs and their evaluation"
while [[ ! -s "$READY" ]]; do
  sleep 300
done

echo "[train] $(date '+%F %T') starting unified learnable GBT ablations"
"$TRAIN" conf &
pid_conf=$!
"$TRAIN" geom &
pid_geom=$!
...
find_run() {
  ...
  find "$ROOT" -maxdepth 1 -type d \
    -name "distill_hardv_legw09_gbt_learnable_${mode}_20260719_*" -print | sort | tail -n 1
}
...
evaluate_mode() {
  ...
  name="gbt_learnable_${mode}_final20"
  ...
  "$EVAL" "$checkpoint" "$name" 2 "/mnt/data/cjyoutput/cmu_v2_eval_${name}_20260720" 1
  "$EVAL" "$checkpoint" "$name" 3 "/mnt/data/cjyoutput/cmu_v3_eval_${name}_20260720" 1
  "$EVAL" "$checkpoint" "$name" 4 "/mnt/data/cjyoutput/cmu_v4_eval_${name}_20260720" 1
}
