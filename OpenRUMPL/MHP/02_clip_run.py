...
def main():
    ...
    ap.add_argument('--image-save-dir', default='/mnt/data/cjydata/mhp_images', type=str)
    ...
    for gid, gname in [(0, 'male'), (1, 'female'), (2, 'neutral')]:
        ...
        bm_dict[gid] = BodyModel(bm_fname=bm_path, num_betas=16).to(device)
    ...
    for clip in bar:
        ...
        try:
            ...
            # 预算 joints_3d (per-frame, 避免下面 view 循环重算20次)
            for tt in range(L):
                joints_3d_clip[tt] = (J_coco @ vertices_all[tt]).astype(np.float32)

            # === Render: view外层 / frame内层 (同时只1个MeshViewer, EGL峰值20→1, 减多worker争用) ===
            # vertices_all 是 per-clip SMPL+H forward 算好的(line ~213); 这里只取[tt]复用, 不重算 mesh
            for v in range(args.n_cameras_per_clip):
                ...
                R_v = cam['R']; t_v = cam['t']
                # Rt 只依赖 view, 算一次 (原代码每frame重算, 浪费)
                Rt = np.eye(4)
                Rt[:3, :3] = -R_v
                Rt[:3, 3] = -t_v.T.flatten()
                Rt[0, :] = -Rt[0, :]  # flip x axis
                for tt in range(L):
                    vertices_t = vertices_all[tt] @ Rt[:3, :3].T + Rt[:3, 3]
                    ...
                    # Project amass joints to 2D
                    joints_cam = world_to_cam(joints_3d_clip[tt][None, :], R_v, cam['T'])
                    joints_2d_amass_clip[tt, v] = cam_to_image(joints_cam, cam['K'])[0]
                    # HRNet
                    try:
                        ...
                        pass
                mv.viewer.delete()  # 用完即删 (同时只1 viewer)
                mv = None

            # === Triangulation (所有view齐后, per frame; 与原版数值等价) ===
            if args.triangulate:
                for tt in range(L):
                    pts_t = joints_2d_mmpose_clip[tt]
                    if not np.isnan(pts_t).all():
                        try:
                            tri = triangulate_poses(cameras_clip, pts_t,
                                                     confs_clip[tt].squeeze(-1),
            ...
            try:
                if mv is not None: mv.viewer.delete()
            except Exception: pass
            n_skipped += 1
            continue
        if n_processed < 3:
            print(f'  [timing] clip{n_processed}: render+HRNet={_time.time()-_t_rh:.1f}s (view外frame内/1viewer)', flush=True)
