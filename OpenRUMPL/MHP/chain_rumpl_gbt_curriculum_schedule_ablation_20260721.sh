...
TRAIN=/home/lixiaob/cjy/OpenRUMPL/MHP/run_rumpl_gbt_curriculum_both_schedule_20260721.sh
...
REPORT=/mnt/data/cjyoutput/rumpl_gbt_curriculum_schedule_ablation_20260721.txt
LOG=/mnt/data/cjyoutput/rumpl_gbt_curriculum_schedule_ablation_20260721.log
V2_RUMPL='3_6=40.37,3_12=46.95,3_13=39.79,3_23=32.30,6_12=67.28,6_13=53.52,6_23=39.39,12_13=59.41,12_23=46.04,13_23=44.08'
RUNS=(3:0 7:1)

exec > >(tee -a "$LOG") 2>&1
echo "[start] $(date '+%F %T') curriculum schedule ablation seed=0"
pids=()
for spec in "${RUNS[@]}"; do
  IFS=: read -r fixed_epochs gpu <<< "$spec"
  "$TRAIN" "$fixed_epochs" 0 "$gpu" &
  pids+=("$!")
done
...
find_run() {
  local fixed_epochs=$1
  find "$ROOT" -maxdepth 1 -type d \
    -name "rumpl_gbt_curriculum${fixed_epochs}_both_seed0_20260721_*" -print | sort | tail -n1
}

evaluate_schedule() {
  local fixed_epochs=$1 gpu=$2 run checkpoint name k
  run=$(find_run "$fixed_epochs")
  checkpoint="$run/final_state.pth.tar"
  name="rumpl_gbt_curriculum${fixed_epochs}_both_seed0_final20"
...
evaluate_schedule 3 0 & e1=$!
evaluate_schedule 7 1 & e2=$!
wait "$e1"
wait "$e2"

: > "$REPORT"
for fixed_epochs in 3 7; do
  name="rumpl_gbt_curriculum${fixed_epochs}_both_seed0_final20"
  for k in 2 3 4 5; do
    ...
    echo "=== fixed_v2_epochs=${fixed_epochs} V${k} ===" >> "$REPORT"
    if [[ $k == 2 ]]; then
