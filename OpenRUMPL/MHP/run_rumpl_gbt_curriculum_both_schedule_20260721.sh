...
if [[ $# -ne 3 ]]; then
  echo "Usage: $0 FIXED_V2_EPOCHS SEED PHYSICAL_GPU" >&2
  exit 2
fi
...
seed=$2
gpu=$3
if (( fixed_epochs < 0 || fixed_epochs > 20 )); then
  echo "FIXED_V2_EPOCHS must be in [0, 20]" >&2
  exit 2
fi
...
echo "[GBT_SCHEDULE] seed=$seed gpu=$gpu fixed_v2_epochs=$fixed_epochs random_k_epochs=$((20 - fixed_epochs))"
exec /home/lixiaob/cjy/rumpl_venv310/bin/python run/train_rumpl.py \
  ...
  --workers 4 \
  --seed "$seed" \
  --exp-name "rumpl_gbt_curriculum${fixed_epochs}_both_seed${seed}_20260721"
