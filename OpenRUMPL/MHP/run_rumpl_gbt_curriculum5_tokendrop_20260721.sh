...
if [[ $# -ne 2 ]]; then
  echo "Usage: $0 TOKEN_DROP_RATE PHYSICAL_GPU" >&2
  exit 2
fi
...
gpu=$2
tag=${rate/./p}
active_epochs=${GBT_TOKEN_DROPOUT_EPOCHS:-0}
exp_suffix=${TOKEN_DROP_TAG:-}
run_seed=${RUN_SEED:-0}
...
export GBT_TOKEN_DROPOUT="$rate" GBT_TOKEN_DROPOUT_EPOCHS="$active_epochs" VFT_FULL_RANDOM_MASK=0
...
echo "[GBT_TOKEN_DROP_RUN] rate=$rate active_epochs=${active_epochs:-all} seed=$run_seed gpu=$gpu fixed_v2_epochs=5"
exec /home/lixiaob/cjy/rumpl_venv310/bin/python run/train_rumpl.py \
  ...
  --workers 4 \
  --seed "$run_seed" \
  --exp-name "rumpl_gbt_curriculum5_tokendrop${tag}${exp_suffix}_seed${run_seed}_20260721"
