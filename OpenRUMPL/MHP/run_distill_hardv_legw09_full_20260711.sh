...
# Single-model candidate after diagnosing [3,12]:
# keep hard-view mining, but make leg down-weighting mild to avoid weak-geometry drift.
export DISTILL_W=1
...
exec /home/lixiaob/cjy/rumpl_venv310/bin/python run/train_rumpl.py \
  ...
  --gpus 0 \
  --workers 8 \
  --exp-name distill_hardv_legw09_full_20260711
