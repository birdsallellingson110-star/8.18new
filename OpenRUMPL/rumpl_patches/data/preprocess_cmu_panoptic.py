...
def preprocess_cmu_mmpose(args):
    ...
    if len(mmpose_data) == 0:
        return []
    gt_2d = np.array(data['joints_2d'])  # GT projection (per-subject), shape (17, 2)
    ...
    if keypoints_standard == 'h36m':
        joints_2d, joints_2d_conf = mmpose2h36m(np.array(pick['keypoints']), np.array(pick['keypoint_scores']))
