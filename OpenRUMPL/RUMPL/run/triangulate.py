...
def parse_args():
    ...
    parser.add_argument(
        '--test-views', type=int, nargs='+', default=None,
        help='override DATASET.TEST_VIEWS (for example: --test-views 1 3)'
    )
    parser.add_argument(
        '--test-on-all-cameras', choices=('config', 'true', 'false'),
        default='config', help='override DATASET.TEST_ON_ALL_CAMERAS'
    )
...
def main():
    ...
    for items, items_with_mmpose in zip(grouping, grouping_with_mmpose):      # all 4 views for one subject
        ...
        # fnames.append([])
        fixed_group_dataset = (
            config.DATASET.TEST_DATASET.startswith('multiview_rich')
            or config.DATASET.TEST_DATASET.startswith('multiview_cmu_panoptic')
            or config.DATASET.TEST_DATASET.startswith('multiview_h36m')
        )
        if run_on_all_cameras and not fixed_group_dataset:
        ...
        # ================== save MPJPE score ==================
        if config.DATASET.TEST_DATASET.startswith('multiview_h36m'):
            cams = ''.join(
                '_{:02d}'.format(test_dataset.db[i]['camera_id'])
                for i in items_chosen
            )
            frame_name = datum['image'].replace('/', '_').replace('.jpg', '') + cams
