...
from core.function_rumpl import inference
...
def parse_args():
    ...
    parser.add_argument(
        '--data-out-dir', help='inference data output directory', type=str,
        default='/mnt/data/cjyoutput/output/inference_rumpl_result.npz'
    )
    parser.add_argument(
...
def main():
    ...
    if args.trained_model:
        logger.info('=> loading model from {}'.format(args.trained_model))
    ...
    inference_dataset = dataset.multiview_inference_openmplposer_rumpl(config)
    ...
    os.makedirs(os.path.dirname(args.data_out_dir), exist_ok=True)
    # print(f'Output directory: {args.data_out_dir}')
    # print(os.path.basename(config.DATASET.INFERENCE_DATA_DIR).replace('.pkl', '.npz'))
    # output_npz_file = os.path.join(args.data_out_dir, os.path.basename(config.DATASET.INFERENCE_DATA_DIR).replace('.pkl', '.npz'))
    output_npz_file = args.data_out_dir
    # evaluate on validation set
    inference(config, inference_loader, inference_dataset, model,
             output_npz_file)
