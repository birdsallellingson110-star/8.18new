...
def nested_view_monotonic_loss(pred_subset, pred_superset, target, margin=0.0):
    """Supervise nested view sets and penalize harmful view additions."""
    if pred_subset.shape != target.shape or pred_superset.shape != target.shape:
        raise ValueError('Nested-view predictions and target must have identical shapes')
    subset_error = torch.norm(pred_subset - target, dim=-1).mean(dim=-1)
    superset_error = torch.norm(pred_superset - target, dim=-1).mean(dim=-1)
    gt_loss = 0.5 * (subset_error.mean() + superset_error.mean())
    monotonic_loss = F.relu(
        superset_error - subset_error.detach() + float(margin)
    ).mean()
    harmful_rate = (superset_error.detach() > subset_error.detach()).float().mean()
    return gt_loss, monotonic_loss, subset_error.detach().mean(), superset_error.detach().mean(), harmful_rate
