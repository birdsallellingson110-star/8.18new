...
class STVFTClipDataset(Dataset):
    ...
    def __init__(self, data_glob, L_window=5, t_target=None, max_views=20, min_oks=0.0,
        ...
        for f in files:
            ...
            for i in range(N):
                ...
                if min_oks > 0:
                    # per-frame OKS(向量化): 窗口中心必须是好帧(随机窗口中心也被保护, 修"半过滤"bug)
                    mm = d['joints_2d_mmpose'][i].astype(np.float64)   # (T,V,17,2)
                    gtm = d['joints_2d_amass'][i].astype(np.float64)
                    area = ((gtm[..., 0].max(-1) - gtm[..., 0].min(-1)) *
                            (gtm[..., 1].max(-1) - gtm[..., 1].min(-1)))  # (T,V)
                    d2 = ((mm - gtm) ** 2).sum(-1)                        # (T,V,17)
                    oks = np.exp(-d2 / ((self._SIGMAS * 2) ** 2) / (area[..., None] + 1e-9) / 2).mean(-1)  # (T,V)
                    oks_frame = np.median(oks, axis=1)                   # (T,) 每帧(视角中位)OKS
                    T = oks_frame.shape[0]
                    valid = [fr for fr in range(H, T - H) if oks_frame[fr] >= min_oks]  # 能当窗口中心的好帧
                    if not valid:
                        n_drop += 1
                        continue
    ...
    def __getitem__(self, idx):
        ...
        # 窗口中心 = 好帧(per-frame OKS过滤过的valid_centers, 保护随机窗口中心); 否则旧逻辑
        vc = c.get('valid_centers')
        if vc:
            center = vc[len(vc) // 2] if self.eval_fixed_window else int(np.random.choice(vc))
            start = center - tt                              # frames[tt]=center=好帧
        elif self.eval_fixed_window:
            start = (T_total - L) // 2
        else:
