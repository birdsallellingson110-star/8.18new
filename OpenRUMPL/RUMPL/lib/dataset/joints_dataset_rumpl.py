...
class JointsDataset_RUMPL(Dataset):
    ...
    def __init__(self, cfg, subset, is_train, transform=None):
        ...
        # H36M keypoints are stored in the distorted image coordinate system,
        # while create_3d_ray_coords implements a pinhole back-projection.  Keep
        # the historical behaviour by default and expose the correction as a
        # controlled experiment rather than silently changing old baselines.
        self.undistort_h36m_points = (
            os.environ.get('RUMPL_UNDISTORT_H36M_POINTS', '0') == '1'
        )
        if self.undistort_h36m_points and not self.no_augmentation:
            raise ValueError(
                'RUMPL_UNDISTORT_H36M_POINTS currently requires '
                'DATASET.NO_AUGMENTATION=true so image crops and rays stay in '
    ...
    def get_rays(self, idx, camera_id=None, camera_setup_to_use=None, shift_room_tri=None):
        ...
        if self.undistort_h36m_points and db_rec['source'] == 'h36m':
            if 'k' not in camera or 'p' not in camera:
                raise KeyError(
                    'H36M point undistortion requires camera radial k and '
                    'tangential p coefficients'
                )
            undistort_k = np.array([
                [float(camera['fx']), 0.0, float(camera['cx'])],
                [0.0, float(camera['fy']), float(camera['cy'])],
                [0.0, 0.0, 1.0],
            ], dtype=np.float64)
            radial = np.asarray(camera['k'], dtype=np.float64).reshape(-1)
            tangential = np.asarray(camera['p'], dtype=np.float64).reshape(-1)
            if radial.size < 3 or tangential.size < 2:
                raise ValueError(
                    'Expected H36M distortion coefficients k[0:3], p[0:2]'
                )
            dist_coeffs = np.array(
                [radial[0], radial[1], tangential[0], tangential[1], radial[2]],
                dtype=np.float64,
            )
            joints = cv2.undistortPoints(
                joints.astype(np.float64)[:, None, :],
                undistort_k,
                dist_coeffs,
                P=undistort_k,
            ).reshape(-1, 2).astype(joints.dtype, copy=False)
