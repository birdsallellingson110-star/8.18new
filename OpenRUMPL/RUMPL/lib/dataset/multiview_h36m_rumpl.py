...
class MultiViewH36M_RUMPL(JointsDataset_RUMPL):
    ...
    def __getitem__(self, idx):
        ...
        items = list(self.grouping[idx])
        # The random-view RUMPL model keeps the first K input tokens.  H36M
        # groups are otherwise always ordered camera 1..4, which would make
        # every K=2/3 training sample use the same camera subset.  Shuffle only
        # during random-view training so K is a genuine random subset while
        # validation/test ordering and metrics remain deterministic.
        if self.is_train and self.max_random_n_views is not None:
